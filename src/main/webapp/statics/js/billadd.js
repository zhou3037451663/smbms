...
$(function () {
    ...
    $.ajax({
        type: "GET",//请求类型
        url: path + "/billproname.json",//请求的url
    ...
    billCode.on("blur", function () {
        ...
        $.ajax({
            type: "GET",//请求类型
            url: path + "/selectbill.json",//请求的url
            data: {billCode: billCode.val()},//请求参数
            dataType: "json",//ajax接口（请求url）返回的数据类型
            success: function (data) {//data：返回数据（json对象）
                if (data.billCode == "exist") {//账号已存在，错误提示
                    validateTip(billCode.next(), {"color": "red"}, imgNo + " 编码已存在", false);
                } else if (data.billCode == "null") {
                    validateTip(billCode.next(), {"color": "red"}, imgNo + " 编码不能为空", false);
                } else {//账号可用，正确提示
                    validateTip(billCode.next(), {"color": "green"}, imgYes + " 编码可以使用", true);
                }
            },
            error: function (data) {//当访问时候，404，500 等非200的错误状态码
